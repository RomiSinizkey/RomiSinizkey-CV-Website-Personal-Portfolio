...
export function EducationSectionContent({ embedded = false }: EducationSectionContentProps) {
  if (embedded) {
    return (
      <div className="mx-auto w-full max-w-5xl">
        ...
        <div className="mt-8 space-y-5">
          {profile.education.map((edu, idx) => (
            <div key={`${edu.institution}-${idx}`} className="homepage-glass-card">
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-white/60">{edu.institution}</p>
                <p className="mt-2 text-sm text-white/50">{edu.years}</p>
              </div>
            </div>
            {edu.details?.length ? (
              <div className="mt-4">
                <ul className="space-y-2">
                  {edu.details.map((detail) => (
                    <li key={detail} className="flex items-start text-sm text-white/70">
