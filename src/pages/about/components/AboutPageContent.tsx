...
function SimpleWheel({ onHoverChange }: { onHoverChange?: (hovering: boolean) => void }) {
  ...
  useEffect(() => {
    const onWindowWheel = (event: WheelEvent) => {
      ...
      const isInside = deltaX * deltaX + deltaY * deltaY <= radius * radius;

      if (!isInside) return;
      event.preventDefault();
      event.stopPropagation();
      setRot((value) => value + event.deltaY * 0.18);
    };
  ...
  const getIconPosition = (index: number) => {
    ...
    const angle = degToRad(baseAngle + rot);
    return {
      x: Math.cos(angle) * rx,
      y: Math.sin(angle) * ry,
    };
  ...
  const buildGearSlicePath = (startDeg: number, endDeg: number, toothOuter: number, toothHalfDeg: number) => {
    ...
    const p1 = polarPoint(innerRadius, startDeg);
    const p2 = polarPoint(outerRadius, startDeg);
    const p3 = polarPoint(toothOuter, mid - toothHalfDeg);
    const p4 = polarPoint(toothOuter, mid + toothHalfDeg);
    const p5 = polarPoint(outerRadius, endDeg);
    const p6 = polarPoint(innerRadius, endDeg);
          ...
          >
            ...
            >
              ...
              {icons.map((icon, index) => {
                ...
                const translateX = Math.cos(degToRad(mid - 90)) * explodeDistance;
                const translateY = Math.sin(degToRad(mid - 90)) * explodeDistance;
                ...
                return (
                  <motion.path
                    ...
                    transition={{ type: "spring", stiffness: 220, damping: 20, delay: index * 0.025 }}
                  />
                );
              })}
            </motion.svg>
...
function CanvaCard({ data, onOpen }: { data: CardData; onOpen: (data: CardData) => void }) {
  return (
    <button
      type="button"
      onClick={() => onOpen(data)}
          ...
          selected ? (
            <div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {selected.modalBullets.map((bullet) => (
                  <div key={bullet} className="rounded-2xl bg-slate-50 border border-slate-200 px-4 py-3">
                    <p className="font-semibold text-slate-800">{bullet}</p>
                  </div>
                ))}
              </div>
